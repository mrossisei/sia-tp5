...
\begin{frame}{Planteo del VAE}
  \begin{columns}[T]
    ...
    \end{column}
    \begin{column}{0.44\textwidth}
      \textbf{Diferencia con el AE:} el encoder produce los
      \emph{parametros} de $q(z|x)=\mathcal{N}(\mu,\sigma^2)$, no $z$ directo.

      \vspace{0.5em}
      $z = \mu + \sigma \odot \varepsilon$,\quad $\varepsilon\!\sim\!\mathcal{N}(0,I)$

      \vspace{0.5em}
      {\footnotesize El KL regulariza el latente hacia $p(z)=\mathcal{N}(0,I)$:
       el espacio queda \textbf{organizado} y permite samplear puntos nuevos
       con sentido.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{El latente se organiza}
  \centering
  \includegraphics[width=0.70\linewidth]{../ej2/results/long_16px/latent_scatter.png}
  \vfill
...
\begin{frame}{Manifold generativo}
  \centering
  \includegraphics[height=0.72\textheight]{manifold_grid.png}
  \vfill
  {\footnotesize Grilla $15\!\times\!15$ en el latente decodificada (percentiles del
   prior $\mathcal{N}(0,I)$). \textbf{Todo punto produce un emoji valido}:
   el espacio latente es continuo.}
\end{frame}
...
\begin{frame}{Generacion: nuevas muestras}
  ...
  {\footnotesize $z\sim\mathcal{N}(0,I)$ decodificados: el VAE genera imagenes
   \textbf{nuevas y reconocibles} del dataset (consigna 2.c $\checkmark$).}
\end{frame}

\begin{frame}{Interpolacion en el latente}
  \centering
  \includegraphics[width=0.92\linewidth]{interpolation.png}
  \vfill
  {\footnotesize Recta entre los centroides $\mu$ de dos clases: la transicion
   es \textbf{suave y continua} gracias al latente organizado.}
\end{frame}
...
\begin{frame}{Estudio 1 --- muestras por dim latente}
  \centering
  {\scriptsize\color{gray} Varia: dim latente $\in\{2,8,16\}$ (resto fijo: ver parametros). Seed representativa.}
  \includegraphics[height=0.74\textheight]{exp_latent_dim_samples.png}
  \vfill
  {\footnotesize Muestras $z\!\sim\!\mathcal{N}(0,I)$ decodificadas. Mayor dim $\Rightarrow$ emojis mas definidos.}
\end{frame}

\begin{frame}{Estudio 1 --- reconstruccion por dim latente}
  \centering
  {\scriptsize\color{gray} Varia: dim latente $\in\{2,8,16\}$ (resto fijo: ver parametros). 5 seeds [0,1,2,3,42].}
  \includegraphics[width=0.62\linewidth]{exp_latent_dim_bars.png}
  \vfill
  {\footnotesize \textbf{8D} es el mejor (MSE$=1{.}61\pm0{.}02$);
   \textbf{2D} sacrifica reconstruccion (MSE$=5{.}05\pm0{.}12$);
   16D no mejora sobre 8D. Elegimos \textbf{2D} para el manifold y la visualizacion directa.}
\end{frame}
...
\begin{frame}{Estudio 2 --- espacio latente por $\beta$}
  ...
  \vfill
  {\footnotesize $\beta$ bajo: clusters dispersos. $\beta$ alto: colapsados al origen.
   \textbf{$\beta=1{.}0$}: estructura clara y separada.}
\end{frame}
...
\begin{frame}{Estudio 3 --- estabilidad entre seeds: parametros}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{table}\small
        \begin{tabular}{ll}
          ...
          $\beta$ / Batch & 1.0 / 64 \\
          Epocas        & 600 \\
          \textbf{Seeds}& \textbf{[42, 7, 123]} \\
          Metrica       & curvas total / rec / KL \\
          \bottomrule
        \end{tabular}
      \end{table}
    \end{column}
    \begin{column}{0.44\textwidth}
      \textit{Variable}: \textbf{seed de inicializacion}.

      \vspace{0.6em}
      \textbf{Pregunta}: ¿el entrenamiento es estable entre distintas
      inicializaciones?

      \vspace{0.4em}
      {\footnotesize El VAE es doblemente estocastico: init aleatoria
       \textbf{y} sampleo $\varepsilon\!\sim\!\mathcal{N}(0,I)$ en cada
       paso. 3 seeds, media $\pm$ std.}
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}{Estudio 3 --- estabilidad entre seeds}
  \centering
  {\scriptsize\color{gray} Varia: seed $\in\{42,7,123\}$ (resto fijo: ver parametros). 3 corridas.}
  \includegraphics[width=0.92\linewidth]{exp_seeds.png}
  \vfill
  {\footnotesize Las 3 seeds convergen a valores muy similares
   (rec$\approx$112, KL$\approx$5.8 al ep.\,600).
   \textbf{El entrenamiento es estable} pese a la doble estocasticidad.}
\end{frame}

\begin{frame}{Estudio 4 --- KL warmup: parametros}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{table}\small
        \begin{tabular}{ll}
...
\begin{frame}{Estudio 4 --- efecto del KL warmup}
  \centering
...
\begin{frame}{Conclusiones EJ2}
  \begin{itemize}\setlength\itemsep{6pt}
    ...
    \item \textbf{Generacion real} (consigna 2.c $\checkmark$): $z\!\sim\!\mathcal{N}(0,I)$
