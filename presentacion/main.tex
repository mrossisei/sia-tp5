...
\begin{frame}{Estudio 3 --- reconstruccion BCE vs MSE: parametros}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{table}\small
        \begin{tabular}{ll}
          ...
          \midrule
          Datos         & emojis $16\!\times\!16$ (960) \\
          Arquitectura  & $[256,64]/[64,256]$, latente 16D \\
          Salida        & logistica (pixeles en $(0,1)$) \\
          Opt / Epocas  & Adam $10^{-3}$ / 400 \\
          \textbf{Seeds}& \textbf{[42, 0, 1]} \\
          Metrica       & rec-MSE (comun), nitidez, KL \\
          \bottomrule
        \end{tabular}
      \end{table}
    \end{column}
    \begin{column}{0.44\textwidth}
      ...
      \vspace{0.4em}
      \textbf{Pregunta}: ¿BCE reconstruye mejor que MSE en imagenes $[0,1]$?

      \vspace{0.3em}
      {\footnotesize Latente \textbf{16D} (no 2D): a 2D el cuello aplasta todo y
       ninguna se ve nitida; con mas capacidad la reconstruccion es legible y la
       diferencia se aprecia.}
...
\begin{frame}{Estudio 3 --- BCE vs MSE: resultados}
  \centering
  {\scriptsize\color{gray} Varia: recon\_loss $\in\{$BCE, MSE$\}$ (resto fijo: ver parametros). 3 seeds [42,0,1].}
    ...
    \begin{column}{0.55\textwidth}
      \begin{table}\small
        \begin{tabular}{ll}
          ...
          Epocas        & 400 \\
          \textbf{Seeds}& \textbf{[42, 0, 1]} \\
          Metrica       & rec-MSE (comun) $+$ n params \\
          \bottomrule
        \end{tabular}
      \end{table}
    \end{column}
...
\begin{frame}{Estudio 4 --- arquitectura: resultados}
  ...
  \vfill
  {\footnotesize rec-MSE baja con la capacidad ($6{.}80\!\to\!5{.}12\!\to\!4{.}34$) con
   \textbf{rendimientos decrecientes} (el cuello 2D limita). La \textbf{profunda} iguala
   a la ancha ($4{.}34$ vs $4{.}37$) con $\sim$mitad de params $\Rightarrow$ profundidad mas eficiente.}
\end{frame}
...
\begin{frame}{Conclusiones EJ2}
  \begin{itemize}\setlength\itemsep{6pt}
    ...
    \item \textbf{2D} equilibra visualizacion y calidad; $>$2D mejora rec sin beneficio visible.
